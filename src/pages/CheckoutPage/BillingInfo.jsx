...
const BillingInfo = ({ emoji, title, info = {} }) => {
  return (
    <section className="billing-Info flex flex-1 flex-col gap-4 rounded-xl border border-white/50 bg-zinc-600 p-8 shadow-xl transition duration-300 hover:-translate-y-4">
      ...
      {info &&
        Object.entries(info).map(([infoKey, infoValue], index) => {
          return (
            <div
              key={index}
              className="flex items-center justify-between border-b border-white/20 py-2"
            >
              <p
